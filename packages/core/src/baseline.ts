...
export function createBaselineDocument(
  ...
  if (
    ...
    throw new Error(
      "Baseline cannot include critical findings unless --allow-critical is explicitly selected.",
    );
  }
  return {
...
export async function loadBaseline(
  ...
  ) as Partial<BaselineDocument>;
  if (
    parsed.schemaVersion !== BASELINE_SCHEMA ||
    !Array.isArray(parsed.fingerprints) ||
    parsed.fingerprints.some((value) => typeof value !== "string")
  ) {
...
export async function writeBaseline(
  ...
  await writeFile(
    path,
    `${JSON.stringify(createBaselineDocument(findings, options), null, 2)}\n`,
    "utf8",
  );
  return path;
}
