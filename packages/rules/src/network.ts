...
function normalizedHostname(raw: string): string {
  return raw.replace(/^\[|\]$/gu, "").split("%", 1)[0]?.toLowerCase() ?? raw.toLowerCase();
}
...
function parseIpv6(address: string): Uint8Array | undefined {
  ...
  const appendGroup = (group: string): boolean => {
    if (group.includes(".")) {
      ...
      if (!parts) return false;
      groups.push((parts[0] ?? 0) * 256 + (parts[1] ?? 0), (parts[2] ?? 0) * 256 + (parts[3] ?? 0));
      return true;
    }
  ...
  for (const group of tail) {
    if (group.includes(".")) {
      ...
      tailGroups.push((parts[0] ?? 0) * 256 + (parts[1] ?? 0), (parts[2] ?? 0) * 256 + (parts[3] ?? 0));
  ...
  const zeroGroups = sections.length === 2 ? 8 - headLength - tailGroups.length : 0;
  if (zeroGroups < 0 || (sections.length === 1 && headLength !== 8)) return undefined;
  const allGroups = [...groups, ...Array.from({ length: zeroGroups }, () => 0), ...tailGroups];
...
function isBlockedIpv6(address: string): boolean {
  ...
  const loopback = allZero || (bytes.slice(0, 15).every((value) => value === 0) && bytes[15] === 1);
  ...
  if (mapped) return isBlockedIpv4(`${bytes[12]}.${bytes[13]}.${bytes[14]}.${bytes[15]}`);
...
function finding(context: RepositoryContext, path: string, url: URL, message: string, evidence: string, severity: Finding["severity"] = "warning"): Finding {
  return {
...
async function checkOne(context: RepositoryContext, path: string, url: URL, timeoutMs: number, resolveHostname: (hostname: string) => Promise<LookupAddress[]>): Promise<Finding | undefined> {
  ...
  try {
    if (await resolvesToBlockedAddress(url, resolveHostname)) {
      return finding(context, path, url, "A network-enabled link check was skipped because the target resolved to an internal or reserved address.", "target blocked by SSRF policy", "info");
    }
