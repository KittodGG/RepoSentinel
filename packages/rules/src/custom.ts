...
function matches(file: RepositoryFile, spec: CustomRuleSpec, expression: RegExp, context: RepositoryContext): boolean {
  if (file.isIgnored || !expression.test(file.relativePath)) return false;
  if (!spec.contentIncludes) return true;
  return context.textCache.get(file.relativePath)?.includes(spec.contentIncludes) ?? false;
}
...
export function loadCustomRules(source: string): RuleDefinition[] {
  ...
  if (!Array.isArray(parsed)) throw new Error("Custom rule registry must be a JSON array.");
  const ids = new Set<string>();
  return parsed.map((item: unknown, index: number) => {
    if (!item || typeof item !== "object" || Array.isArray(item)) throw new Error(`Custom rule ${index} must be an object.`);
    const record = item as Record<string, unknown>;
    const id = assertString(record.id, "id", index);
    if (!/^custom\.[a-z0-9][a-z0-9._-]*$/u.test(id)) throw new Error(`Custom rule ${id} must use the custom.<name> ID format.`);
    if (ids.has(id)) throw new Error(`Duplicate custom rule ID: ${id}.`);
    ids.add(id);
    const severity = assertString(record.severity, "severity", index) as Severity;
    if (!severities.has(severity)) throw new Error(`Custom rule ${id} has unsupported severity: ${severity}.`);
    ...
    const selectedProfiles = record.profiles === undefined
      ? undefined
      : Array.isArray(record.profiles)
        ? record.profiles.map((profile) => assertString(profile, "profile", index))
