...
function normalizeReport(value: unknown, sourceFile: string): DashboardReport {
  if (!value || typeof value !== "object")
    throw new Error(`Report ${sourceFile} is not a JSON object.`);
  const record = value as Record<string, unknown>;
  if (record.schemaVersion !== "reposentinel.report/v1")
    throw new Error(`Report ${sourceFile} has an unsupported schema.`);
  if (
  ...
  const findings = Array.isArray(record.findings)
    ? record.findings
        ...
        .filter(
          (finding): finding is DashboardFinding =>
...
export function parseDashboardReport(
  source: string,
...
export function renderDashboardReport(
  ...
  const average =
    ordered.length === 0
      ? 0
      : Math.round(
          ordered.reduce((sum, report) => sum + report.score, 0) /
            ordered.length,
        );
  ...
  const rows =
    ordered.length === 0
      ...
      : ordered
          .map(
            (report) =>
              `<tr><td><strong>${escapeHtml(report.repository)}</strong></td><td><code>${escapeHtml(report.profile)}</code></td><td><strong class="score-${report.score >= 90 ? "good" : report.score >= 75 ? "mid" : "risk"}">${report.score}/100</strong></td><td>${escapeHtml(report.status)}</td><td>${report.findings.length}</td></tr>`,
          )
          .join("\n");
  return `<!doctype html>
