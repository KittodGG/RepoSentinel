...
packageJson.dependencies = Object.fromEntries(
  Object.entries(packageJson.dependencies ?? {}).filter(([name]) => !name.startsWith("@reposentinel/"))
);
await writeFile(join(staging, "package.json"), `${JSON.stringify(packageJson, null, 2)}\n`);

await execFileAsync("npm", ["pack", "--ignore-scripts", "--pack-destination", artifactDir], {
  cwd: staging,
